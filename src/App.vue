...
position-absolute top-0 end-0 me-3 d-flex align-items-center">
            ...
            <button class="btn"
            :class="{'btn-secondary': theme==='light','btn-outline-primary': theme==='dark'}"
            v-if="loginState" @click="signOut">Sign Out</button>
            <router-link class="btn"
            :class="{'btn-secondary': theme==='light','btn-outline-primary': theme==='dark'}"
            to="/login"
            v-else-if="!loginState && !isLogin">Login</router-link>
          </div>
      </div>
    </nav>
  </header>
...
import sunIcon from './assets/images/components/sun.png';
import moonIcon from './assets/images/components/moon.png';
